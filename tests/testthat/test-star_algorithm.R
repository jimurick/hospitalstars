...
test_that("Algorithm v202104 agrees with SAS", {
  ...
  r_df <-
    ...
    hospitalstars::compute_star_scores(star_version = "v202104")
...
test_that("Algorithm v202207 agrees with SAS", {
  ...
  r_df <-
    ...
    hospitalstars::compute_star_scores(star_version = "v202207")
...
test_that("Algorithm v202307 agrees with SAS", {

  # Output from the SAS package released July 2022, starting from the sample
  # input data. The SAS output was copied into the dataframe `output_df`
  sas_df <- hospitalstars::sas_package$v202307$output_df

  # Output using this R package with the same input data
  r_df <-
    hospitalstars::sas_package$v202307$input_df %>%
    hospitalstars::compute_star_scores() %>%
    # For some reason, the SAS package drops a lot of hospitals in its output
    # this year.
    filter(PROVIDER_ID %in% sas_df$PROVIDER_ID)
